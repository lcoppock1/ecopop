...
export async function checkAndAwardBadges(gameStats) {
  const progress = await loadProgress();
  const badges = progress.earnedBadges || {};
  const materialStats = progress.materialStats || { 1: 0, 2: 0, 3: 0, 4: 0 };
  const newBadges = [];
  ...
  if (!badges.earth_warrior && progress.totalEcoScore >= 10000) {
  ...
  if (!badges.combo_king && gameStats.maxCombo >= 5) {
